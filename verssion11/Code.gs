...
const TOKEN_TTL_MINUTES = 60 * 24 * 7; // 7 days

const SHEET_TRACKER = "Tracker";
const SHEET_TUTORIAL = "Tutorial";
const SHEET_TUTORIAL_FALLBACK = "Tutotrial";
...
function createSession_(principal, type) {
  const sh = sh_(SHEET_SESSIONS);
  const token = Utilities.getUuid().replace(/-/g, "");
  const createdAt = new Date();
  const expiresAt = new Date(createdAt.getTime() + TOKEN_TTL_MINUTES * 60 * 1000);
...
function getTrackerRows_(sheetName) {
  sheetName = String(sheetName || SHEET_TRACKER).trim();
...
function getTutorialRows_() {
  const sh = optSh_(SHEET_TUTORIAL) || optSh_(SHEET_TUTORIAL_FALLBACK);
  if (!sh) return { ok: false, error: "Missing sheet tab: " + SHEET_TUTORIAL };
  return { ok: true, rows: rowObjectsFromSheet_(sh) };
}
...
function doGet(e) {
  ...
  if (action === "tutorial_list") {
    ...
    return jsonOut(getTutorialRows_());
  }
  ...
  if (action === "tracker") {
    ...
    const sheetName = (e.parameter && e.parameter.sheet) || SHEET_TRACKER;
